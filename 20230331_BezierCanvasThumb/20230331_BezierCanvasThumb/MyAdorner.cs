...
namespace _20230331_BezierCanvasThumb
{
    public class MyAdorner : Adorner
    {
        ...
        public GeometricShape MyTargetBezier { get; private set; }


        public MyAdorner(GeometricShape adornedElement) : base(adornedElement)
        {
        ...
        {
            SetBinding(MyPointsProperty, new Binding() { Source = MyTargetBezier, Path = new PropertyPath(GeometricShape.MyPointsProperty) });
        ...
        private void SetThumbLocate(Thumb thumb, Point pp)
        {
            Canvas.SetLeft(thumb, pp.X);
            Canvas.SetTop(thumb, pp.Y);
        }
        private void SetThumbLocateCenter(Thumb thumb, Point pp)
        {
            Canvas.SetLeft(thumb, pp.X - (MyAnchorThumbSize / 2.0));
            Canvas.SetTop(thumb, pp.Y - (MyAnchorThumbSize / 2.0));
        }
        ...
        public void FixThumbsLocate()
        {
            ...
            {
                Point pp = MyPoints[i];
                SetThumbLocate(MyThumbList[i], pp);
            }
        ...
        private void Thumb_DragDelta(object sender, DragDeltaEventArgs e)
        {
            if (sender is Thumb thumb)
            {
                ...
                SetThumbLocate(thumb, MyPoints[ii]);
            }
        }
    }
